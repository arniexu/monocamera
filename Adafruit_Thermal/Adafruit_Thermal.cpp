...
// Constructor
Adafruit_Thermal :: Adafruit_Thermal (PinName tx, PinName rx, int baud,uint8_t dtr) : Serial(tx, rx),dtrPin (dtr) {
	Serial::baud(baud);
  dtrEnabled = false;
}
...
#define printBreakTime  2 // 500 uS
  ...
  // Enable DTR pin if requested NEVER Enable DTR
  if(dtrPin < 255) {
...
//modified by arniexu
uint8_t Adafruit_Thermal::toupper(uint8_t value){
...
void Adafruit_Thermal::printBitmap(
 int w, int h, const uint8_t *bitmap, bool inverse) {
		int rowBytes, rowBytesClipped, x, y, i;

		rowBytes        = (w + 7) / 8; // Round up to next byte boundary
		rowBytesClipped = (rowBytes >= 48) ? 48 : rowBytes; // 384 pixels max width

    writeBytes(ASCII_DC2, '*', h, rowBytesClipped);
...
void Adafruit_Thermal::printBitmap(int w, int h, uint8_t *bitmap) {
	
}

void Adafruit_Thermal::printBitmap(const char *file) {
}
